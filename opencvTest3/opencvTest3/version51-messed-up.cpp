	...
	for ( currJVal=0; currJVal < numJVals; currJVal++)
	{
		...
		if(gameChoice == 1) // if cashburst, ... .
		{
			...
			if( currJVal == 0 ) // for j0 (assuming j0 is unique)
			{
				...
				{
					char charASCII=0;
					string trainPath = chooseFontSizeChar("../src/trainCashburstJ0/", i, charASCII);
					train(toFile, trainPath, fDatabase, charASCII);
				}
			}
			...
			if( currJVal == 1 ) 
			{
				...
				{
					char charASCII=0;
					string trainPath = chooseFontSizeChar("../src/trainCashburstJ14/", i, charASCII);
					train(toFile, trainPath, fDatabase, charASCII);
				}
			}	
		}
		...
		if(gameChoice == 2) // if InstantRiches, ... .
		{
			...
			if( currJVal == 0 ) // for j0 (assuming j0 is unique)
			{
				...
				{
					char charASCII=0;
					string trainPath = chooseFontSizeChar("../src/trainInstRichJ0/", i, charASCII);
					train(toFile, trainPath, fDatabase, charASCII);
				}
			}
			...
			if( currJVal == 1 ) 
			{
				...
				{
					char charASCII=0;
					string trainPath = chooseFontSizeChar("../src/trainInstRichJ14/", i, charASCII);
					train(toFile, trainPath, fDatabase, charASCII);
				}
			}	
		}
...
int computeNumContourPts(Mat imgGrayComp, vector< vector<int> > & imgPxlsFlags, bool testFlag)
{
	...
	while( imgGrayComp.ptr(j)[i] == 0 )
	{
		cout << vertMiddle << "\n";
		cout << rightOfChar << "\n";
		cout << imgGrayComp.ptr(vertMiddle)[rightOfChar];
		i++;
	...
	// ziy+dirs[dir][1] - y of neighbor
	// zix+dirs[dir][0] - x of neighbor
	...
	cout << "\nHere 1y: " << ziy+dirs[dir][1];
	cout << "\nHere 1x: " << zix+dirs[dir][0];
	cout << "\nHere: " << imgGrayComp.ptr(ziy+dirs[dir][1])[(zix+dirs[dir][0])];
	cout << "\nHere: " << imgPxlsFlags[ ziy+dirs[dir][1] ][ zix+dirs[dir][0] ];
		...
		int numPoints, vector< vector<int> > & imgPxlsFlags, bool testFlag)
{
	...
	while ( currPoint!=(numPoints-1) )
	{
		...
		while (next)
		{
			cout << "\nbum 1a: " << currPoint << endl;
			cout << "\nbum 1b: " << ziy+dirs[dir][1] << endl;
			cout << "\nbum 1c: " << zix+dirs[dir][0] << endl;
...
void detectNextAndMatch(ofstream & toFile, Mat imgDtct, Mat imgWithContours, float fDatabase[][charsToClassify])
{
	...
	*/
	...
	//while( imgDtct.ptr(vertMiddle)[rightOfChar] == 255 ) rightOfChar++;
	//while( imgDtct.ptr(vertMiddle)[rightOfChar] == 0 ) rightOfChar++;
	
	//cout << endl << " : " << rightOfChar << endl << endl;
