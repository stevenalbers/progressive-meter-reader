...
int main()
{
	...
	else if (gameChoice == 1) 
	{
		...
		// ( int jValID, int topLeftX, int topLeftY, int bottomRightX, int bottomRightY ):
	...
	{
		if( currJVal == 1 ) // once and for 1-4, train on another set.
		{
...
int computeNumContourPts(Mat imgGrayComp, vector< vector<int> > & imgPxlsFlags, bool testFlag)
{
	...
	threshold(imgGrayComp, imgDil, thresh, 255, THRESH_BINARY); // threshold() used to copy one image into another
