/*
 * Authors:		Andrey Gaganov (image processing, I/O, misc.)
 *				George Bebis (recognition)
 *				Dan Diggins (Otsu's method for thresholding)
 * 
 * Date:		4/27/2014
 *
 */
...
int main()
{
	...
	threshold(imageGray, imgWithContours, 255, 255, THRESH_BINARY); // threshold() used to copy one image into another
	
	// false premature thresholding to avoid runtime error (action will be "undone" in the future):
	threshold(imageGray, imgThreshDilErd, threshVal, 255, THRESH_BINARY);

	// thresholing the openCV way - with a hand-picked global threshold value:
	...
	for ( currJVal=0; currJVal < numJVals; currJVal++)
	{
		...
		if(gameChoice == 1) // if cashburst, ... .
		{
			...
			{
				// true thresholding here:
				//threshVal = otsuThresholding(imageGray);
				//threshold(imageGray, imgThreshDilErd, threshVal, 255, THRESH_BINARY);
				//function with thresh-mapped regions ()

				threshold(imageGray, imgDummy, otsuThresholding(imageGray), 255, THRESH_BINARY);
				imwrite( "../src/outThresh-J0.png" , imgDummy); // saves the chosen output image
			...
			if( currJVal == 1 ) 
			{
				...
				threshold(imageGray, imgDummy, otsuThresholding(imageGray), 255, THRESH_BINARY);
				imwrite( "../src/outThresh-J1.png" , imgDummy); // saves the chosen output image
			...
			if( currJVal == 2 ) 
			{
				...
				threshold(imageGray, imgDummy, otsuThresholding(imageGray), 255, THRESH_BINARY);
				imwrite( "../src/outThresh-J2.png" , imgDummy); // saves the chosen output image
			...
			if( currJVal == 3 ) 
			{
				...
				threshold(imageGray, imgDummy, otsuThresholding(imageGray), 255, THRESH_BINARY);
				imwrite( "../src/outThresh-J3.png" , imgDummy); // saves the chosen output image
			...
			if( currJVal == 4 ) 
			{
				...
				threshold(imageGray, imgDummy, otsuThresholding(imageGray), 255, THRESH_BINARY);
				imwrite( "../src/outThresh-J4.png" , imgDummy); // saves the chosen output image
		...
		cout << endl << endl << "Suggested threshold: " << otsuThresholding(imageGray) << endl << endl;

		// set up boundaries FOR THE GIVEN REGION to perform char-by-char detection:
		prepareDtxnVars( currJVal ); 

		while(!reachedEndOfRgn)
		{
			// using threshold() to make a copy of the testing image for every region:
			threshold(imgThreshDilErd, imgTest, 128, 255, THRESH_BINARY);
			// send the copy to this function:
		...
		char whichChar)
{
	...
	cvtColor(image, imgTrain, COLOR_BGR2GRAY );
	//! applies fixed threshold to the image
	threshold(imgTrain, imgTrain, threshVal, 255, THRESH_BINARY);

	// ======= time to find the 4 moments for the char: =======
	// pre-compute no_points(size)(N):
	int numPoints = computeNumContourPts(imgTrain, imgPxlsFlags, 0); // might encounter inf-loop
	...
	writeContourCoords(imgTrain, arrayPoints, numPoints, imgPxlsFlags, 0);
	clearFlags(imgPxlsFlags, image); // clear after allocation
...
/*
 *	Otsu's method for thresholding (per every j-value-region).
 *
 *	Author: Dan Diggins (http://www.dandiggins.co.uk/arlib-9.html)
 */
int otsuThresholding(Mat imageGray)
{
	Mat imgDummy;
	...
	int optThresh = 0;		// optimal threshold value
	...
	// computing threshold based on the region's pxl values:
	int height = jValRgns[currJVal].bottomRight.y - jValRgns[currJVal].topLeft.y;
	int width = jValRgns[currJVal].bottomRight.x - jValRgns[currJVal].topLeft.x;
	int tLx = jValRgns[currJVal].topLeft.x;
	int tLy = jValRgns[currJVal].topLeft.y;
	
	// histogram elements assignment:
	for(int hi=0; hi<=255; hi++)
		histogram[hi] = 0;

	// incr. the pxl val amounts on the histogram:
	for(int a=0; a<height; a++)
		for(int b=0; b<width; b++)
			histogram[imageGray.ptr( tLy + a )[ tLx + b ]]++;
	...
	// Normalized histogram ready. Now we plot it on a blank image.
	// 1) Copy:
	threshold(imageGray, imgDummy, 255, 255, THRESH_BINARY);
	// 2) Plot:
	//		a) Find max num of points of same pxl val:
	float maxHist = 0;
	for (int I=0; I<=255; I++)
	{
	...
	for (int I=0; I<=255; I++)
	{
		...
		int pxlValAmtScld = 1.00*histNormalized[I]*heightFloor/maxHist;
		// if pxlValAmt = 0,	we assign to length-address;	=> need to bring height down by 1
		// if pxlValAmt = max,	we assign to element 0;			=> OK. 
		imgDummy.ptr( heightFloor - pxlValAmtScld )[imgDummy.size().width * I/255] = 255;
	}
	
	dilate(imgDummy, imgDummy, Mat(), Point(-1,-1), 2 ); // the plot thickens.
	...
	optThresh = (int)sqrt(work3);
	
	// Marking the optimal threshold on the histogram:
	for(int d=0; d<imgDummy.size().height; d++)
	{
		imgDummy.ptr(d)[ imgDummy.size().width*optThresh/255 ] = 128;
	}

	// 3) Write:
	if(currJVal==0) imwrite( "../src/outHist-J0.png" , imgDummy);
	if(currJVal==1) imwrite( "../src/outHist-J1.png" , imgDummy);
	if(currJVal==2) imwrite( "../src/outHist-J2.png" , imgDummy);
	if(currJVal==3) imwrite( "../src/outHist-J3.png" , imgDummy);
	if(currJVal==4) imwrite( "../src/outHist-J4.png" , imgDummy);


	// Convert the final value to an integer, return the threshold:
	return optThresh;
}
