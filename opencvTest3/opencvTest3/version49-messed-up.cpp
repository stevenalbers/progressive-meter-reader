		...
		char whichChar)
{
	...
	// ------------------------------------------------------------------------
	// displays distances for centroidal profiles:
	/*
	cout << endl;
	if (whichChar == '9')
	{
...
void detectNextAndMatch(Mat imgDtct, Mat imgWithContours, float fDatabase[][charsToClassify])
{
	...
	*/
	...
	for(int z=0; z < numPoints; z++) // go through every point
	{
		if(rightOfChar < arrayPoints[z].x) // keep updating the rightmost x
		{
			// update the rightmost x:
