...
int main()
{
	...
	//threshold(imageGray, imageGray, thresh, 255, THRESH_BINARY);	- allows for too much white
	//threshNick("../src/cd.jpg");									- probably old code
	customThresh1D(imageGray); // applies fixed threshold to the image
...
int computeNumContourPts(Mat imgGrayComp, bool testFlag)
{
	...
	else
	{
		// ======= to prevent from getting stuck at white noise pixels: =======
		//dilation x1 (increases workload):
		dilate(imgGrayComp, imgGrayComp, Mat(), Point(-1,-1), 3); // recommended # of steps: 3
		//erosion x1 (decreases workload):
		erode(imgGrayComp, imgGrayComp, Mat(), Point(-1,-1), 2); // recommended # of steps: 2
	}
